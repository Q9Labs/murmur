...
describe("worker routes", () => {
  it("answers preflight requests with CORS headers", async () => {
    const response = await worker.fetch(
      new Request("https://worker.example/v2/session", { method: "OPTIONS" }),
      {},
    );
  ...
  it.each([
  ...
  ])("serves the tracked %s landing page", async (path, campaign) => {
    const response = await worker.fetch(
      new Request(`https://worker.example${path}`),
      {},
    );
    ...
    expect(body).toContain(`utm_campaign=${campaign}`);
  });
  ...
  it("rejects session creation when OpenAI Realtime is not configured", async () => {
    const response = await worker.fetch(
  ...
  it("does not silently replace the installed v1 session contract", async () => {
    const response = await worker.fetch(
      new Request("https://worker.example/v1/session", { method: "POST" }),
    ...
    try {
      const response = await worker.fetch(
        new Request("https://worker.example/v2/session", {
          body: JSON.stringify({
            acquisition: {
              campaign: "Gulf Summer 2026!",
              content: "caption demo",
              ignored: "raw content must not be logged",
              landing: "live-translation-for-travel",
        ...
        { OPENAI_API_KEY: "test_key", SESSION_HASH_SALT: "measurement_salt" },
      );

      expect(response.status).toBe(200);
      const sessionEvent = logSpy.mock.calls
      ...
      expect(sessionEvent?.hashed_install_id).toMatch(/^[a-f0-9]{64}$/);
      expect(JSON.stringify(sessionEvent)).not.toContain(installId);
      expect(JSON.stringify(sessionEvent)).not.toContain("raw content");
    } finally {
      logSpy.mockRestore();
    }
  });

  it("validates session requests before OpenAI Realtime configuration", async () => {
  ...
  it("creates and closes an OpenAI realtime session", async () => {
    ...
    const closeResponse = await worker.fetch(
      new Request(
        `https://worker.example/v2/session/${session.app_session_id}/stop`,
        { method: "POST" },
      ),
      {},
    );
  ...
  it("returns not found for removed legacy routes", async () => {
    const response = await worker.fetch(
