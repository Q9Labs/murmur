...
describe("provider-neutral realtime socket", () => {
  beforeEach(() => {
  ...
  it("closes invalid, unconfigured, and unknown sessions", async () => {
    ...
    await proxyRealtimeSession(
      new Request("https://worker.test/v1/realtime"),
    ...
    await proxyRealtimeSession(
      new Request(
        "https://worker.test/v1/realtime?app_session_id=session&target_language=ar",
      ),
    ...
    await proxyRealtimeSession(
      new Request(
        "https://worker.test/v1/realtime?app_session_id=missing&target_language=ar",
      ),
  ...
  it("reports provider connection failures without leaking details", async () => {
    ...
    await proxyRealtimeSession(
      new Request(
        `https://worker.test/v1/realtime?app_session_id=${appSessionId}&target_language=ar`,
      ),
  ...
  it("proxies audio, transcripts, translated audio, close, and errors", async () => {
    ...
    await proxyRealtimeSession(
      new Request(
        `https://worker.test/v1/realtime?app_session_id=${appSessionId}&target_language=pt-BR`,
      ),
      client as unknown as WorkerWebSocket,
      {
        OPENAI_API_KEY: "test_key",
    ...
    expect(JSON.parse(String(upstream.sent[0]))).toMatchObject({
      session: { audio: { output: { language: "pt" } } },
      type: "session.update",
    });
