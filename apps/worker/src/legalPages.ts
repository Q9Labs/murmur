...
export const legalPages: Record<string, Page> = {
  ...
  "/privacy": {
    ...
    html: `
      ...
      <p>Anonymous Analytics is on by default and can be turned off in Settings. Product analytics events pass through Murmur's Worker relay, which validates event names and properties before forwarding them to PostHog. Events may describe app and build version, platform, language pair, onboarding steps, plan and offer interactions, sign-in method, session outcome, background use, capture source, broad network type, timing, duration, bounded error categories, and install source. The relay does not include your email address, provider account identifier, or credentials.</p>
      <p>Session replay uses the PostHog SDK directly from the device, not Murmur's Worker relay. PostHog therefore sees the device IP address on replay network traffic. Replays do not include microphone audio; all text, text inputs, and images are masked. When Anonymous Analytics is turned off, the app stops session recording and opts the replay client out. Essential sanitized crash and error monitoring can continue.</p>
      ...
      <ul>
        ...
        <li>Decline AI session insights or turn them off in Settings to stop collecting translated text for future insights. Existing insights remain until account deletion or the 24-month retention job.</li>
  ...
  "/terms": {
    ...
    html: `
      ...
      <p>AI Session Insights are optional and separate from live translation. Only translated text from a consented session is collected for this feature. For a session with at least 30 seconds of translation, Murmur sends the text to OpenRouter to create a short derived insight, then discards the text without storing it as a transcript. Murmur deletes each insight after 24 months in a daily job or earlier when the associated account is deleted. Turning off insights stops collection for future sessions but does not delete existing insights. The Privacy Policy explains the consent and deletion controls.</p>
  ...
  "/support": {
    ...
    html: `
      ...
      <p>Only translated text from a consented session is collected for Session Insights. Turning off Session Insights stops collection for future sessions but does not delete existing insights. A daily job deletes insights after 24 months; account deletion removes associated insights sooner.</p>
