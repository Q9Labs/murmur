...
export const legalPages: Record<string, Page> = {
  ...
  "/privacy": {
    ...
    html: `
      ...
      <p><strong>Anonymous install and session metadata.</strong> The app also creates an anonymous install identifier in platform secure storage. The Worker hashes this identifier for abuse prevention, diagnostics, and pseudonymous session measurement. The app includes controls to reset the install identity and delete local Murmur data.</p>
      ...
      <h2>Retention</h2>
      <p>Murmur does not retain audio, transcript history, or translated caption history by default. Account records remain until account deletion. Entitlement, store transaction, usage, renewal, and refund ledger records are retained as needed for service integrity, fraud prevention, financial reconciliation, and legal obligations, with direct identifiers removed when the account is deleted. Analytics, diagnostics, campaign, and rate-limit metadata is retained only as needed for product measurement, abuse prevention, debugging, and service operation, then deleted or anonymized under Murmur's provider retention settings.</p>
      <h2>Your Choices</h2>
      <ul>
