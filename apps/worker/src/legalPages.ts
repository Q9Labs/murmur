...
export const legalPages: Record<string, Page> = {
  ...
  "/privacy": {
    ...
    html: `
      ...
      <p>You may save a purchase by signing in with Apple, Google, or email. For Apple sign-in, the app requests your email address and full name; Murmur receives your email address and your name if you choose to share it and Apple provides it on first sign-in, along with Apple's stable account identifier. For Google sign-in, Murmur receives your email address and Google's stable account identifier. Email sign-in uses your email address to deliver a one-time code. Murmur does not receive your Apple or Google password.</p>
  ...
  "/support": {
    ...
    html: `
      ...
      <p>Murmur creates a random guest customer account when you first use the app. You can purchase, restore, and reconcile purchases as a guest. You may add Apple, Google, or email sign-in and save a guest purchase to that account. Murmur does not keep cloud transcript history. For eligible paid users, Murmur saves translated captions locally on the device only.</p>
