...
Session replay uses the PostHog SDK directly from the device, not Murmur's Worker relay. PostHog therefore sees the device IP address on replay network traffic. Replays do not include microphone audio; all text, text inputs, and images are masked. When Anonymous Analytics is turned off, the app stops session recording and opts the replay client out. Essential sanitized crash and error monitoring can continue.
