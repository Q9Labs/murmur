...
- **“Works offline,” “on-device translation,” or “your audio never leaves your phone”:** live sessions require the Worker and OpenAI Realtime, and selected live audio passes through the Worker to that service. Sources: `apps/mobile/src/lib/live-translation/workerApi.ts`, `apps/worker/src/sockets/realtime.ts`, `docs/legal/privacy-policy.md`.
...
- **“Unlimited use”:** the Worker enforces session and translation limits, including a 5-minute maximum session, one active session per install, and per-install session caps. Source: `apps/worker/src/limits.ts`.
