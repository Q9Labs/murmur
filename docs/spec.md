...
The GPT Realtime contract uses `/v2` so it does not silently mutate the installed
client contract at `/v1`. A `/v1/session` create request returns
`client_upgrade_required`; production rollout must keep the earlier deployment
available until supported clients have moved to `/v2`.
