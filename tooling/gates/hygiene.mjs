...
const commitScript = readFileSync(join(repoRoot, "tooling", "gates", "commit.sh"), "utf8");
...
for (const name of referencedScripts) {
  if (!scripts[name]) {
    failures.push(`tooling/gates/commit.sh references missing package script "${name}"`);
  }

  if (forbiddenHookScripts.has(name)) {
    failures.push(`tooling/gates/commit.sh must not run live/release lane "${name}"`);
  }
}

if (/\bfastlane\b/i.test(commitScript)) {
  failures.push("tooling/gates/commit.sh must not run Fastlane lanes");
}

if (/\b(store:live|store:preflight|store:submission-readiness|store:launch-ready)\b/.test(commitScript)) {
  failures.push("tooling/gates/commit.sh must not run live or release store lanes");
}
