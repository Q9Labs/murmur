...
export type BillingProductCode =
  ...
  | "pro_monthly_lite" | "pro_annual_lite"
  | "credits_60" | "credits_30_lite" | "credits_300";
...
export function findBillingProduct(
  ...
  return billingProducts.find((product) => provider === "apple"
...
export function isPersonalOfferProduct(productId: string, offerId: string | null = null): boolean {
  return billingProducts.some((product) => product.personalOffer === true &&
    (product.appleProductId === productId ||
      (product.googleProductId === productId && product.googleOfferId === offerId)));
}
