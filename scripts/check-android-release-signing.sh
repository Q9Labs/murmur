...
if [[ ! -f "$BUILD_GRADLE" ]]; then
  echo "Missing Android Gradle project at $BUILD_GRADLE. Run Android prebuild before building a signed release." >&2
  exit 1
fi
...
for pattern in "${required_patterns[@]}"; do
  if ! grep -q "$pattern" "$BUILD_GRADLE"; then
    echo "Android release signing guard is missing from android/app/build.gradle: $pattern" >&2
